...
parser = argparse.ArgumentParser(
    description="Run iRODS plugin test hooks in a consistent environment."
)
...
parser.add_argument(
    ...
    nargs="?",
    default=None,
    const="/var/lib/irods/test-reports",
...
try:
    ...
    for c in containers:
        archive.copy_archive_to_container(
            c, archive.create_archive([plugin_package_directory], args.plugin_name)
        )

    options = ["--built_packages_root_directory", plugin_package_directory]

    rc = test_utils.run_plugin_tests(
        containers,
        args.plugin_name,
        args.test_hook,
        args.tests,
    ...
    if args.extra_logs_path:
        try:
            logs.collect_logs(
                ...
                ctx.irods_containers(),
                output_directory,
                logfile_path=args.extra_logs_path,
            )
        except docker.errors.NotFound:
            logging.warning(
                "Path in container not found for --extra-logs-path {!r}".format(
