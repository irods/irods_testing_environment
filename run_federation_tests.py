...
if __name__ == "__main__":
    ...
    try:
        if args.do_setup:
            ...
            containers = ctx.compose_project.up(
                scale_override={
        ...
        remote_container = ctx.docker_client.containers.get(
            context.container_name(
                ctx.compose_project.name, context.irods_catalog_provider_service()
            )
        )
        ...
        if args.do_setup:
            irods_config.configure_irods_federation_testing(
                ctx, zone_info_list[0], zone_info_list[1]
            )
