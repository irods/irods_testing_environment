...
def generate_ssl_self_signed_certificate(key, directory=None):
    ...
    subject = issuer = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "iRODS Consortium"),
        ]
    )
...
def configure_ssl_on_server(
    ...
    if (
        execute.execute_command(container, "{} stop".format(irodsctl), user="irods")
        != 0
    ):
        raise RuntimeError(
            "failed to stop iRODS server before SSL configuration [{}]".format(
                container.name
            )
        )
    ...
    service_account_irods_env = os.path.join(
        context.irods_home(), ".irods", "irods_environment.json"
    )
    ...
    if (
        execute.execute_command(container, "{} start".format(irodsctl), user="irods")
        != 0
    ):
        raise RuntimeError(
            "failed to start iRODS server after SSL configuration [{}]".format(
                container.name
            )
        )
...
def configure_ssl_in_zone(docker_client, compose_project):
    ...
    try:
        ...
        csps = compose_project.containers(
            service_names=[context.irods_catalog_provider_service()]
        )
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for f in concurrent.futures.as_completed(futures_to_containers):
                ...
                try:
                    ...
                    logging.error(
                        "exception raised while configuring SSL [{}]".format(
                            container.name
                        )
                    )
                    logging.error(e)
        ...
        cscs = compose_project.containers(
