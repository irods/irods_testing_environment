class context(object):
    ...
    def platform(self, platform_service_name=None, platform_service_instance=1):
        ...
        if not self.platform_image_tag:
            self.platform_image_tag = base_image(
    ...
    def database(self, database_service_instance=1):
        ...
        if not self.database_image_tag:
            self.database_image_tag = base_image(
                self.docker_client.containers.get(
    ...
    def irods_containers(self):
        ...
        return [
            c
            for c in self.compose_project.containers()
...
def sanitize(repo_or_tag):
    ...
    """
    return repo_or_tag.replace(".", "").replace(":", "").replace("/", "")
...
def base_image(container, tag=0):
    ...
    return [
        image
        for image in container.client.images.get(
            container.client.api.inspect_container(container.name)["Config"]["Image"]
        ).history()
        if "<missing>" not in image["Id"]
    ][-1]["Tags"][tag]
...
def container_ip(container, network_name=None):
    ...
    """
    return container.client.api.inspect_container(container.name)["NetworkSettings"][
        "Networks"
    ][network_name or "_".join([project_name(container.name), "default"])]["IPAddress"]
...
def irods_catalog_provider_container(project_name, service_instance=1):
    ...
    return container_name(
        project_name, irods_catalog_provider_service(), service_instance
    )
...
def irods_catalog_consumer_container(project_name, service_instance=1):
    ...
    """
    return container_name(
        project_name, irods_catalog_consumer_service(), service_instance
    )
...
def irods_catalog_database_container(project_name, service_instance=1):
    ...
    """
    return container_name(
        project_name, irods_catalog_database_service(), service_instance
    )
...
def is_irods_server_in_local_zone(container, local_zone):
    ...
    raise NotImplementedError(
        "service name is not supported [{}]".format(container.name)
    )
