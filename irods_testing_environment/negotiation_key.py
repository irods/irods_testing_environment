...
def backup_file(container, file_path):
    ...
    if (
        execute.execute_command(
            container, "cp {} {}".format(file_path, backup_file_path)
        )
        != 0
    ):
        raise RuntimeError(
            "failed to backup [{}] [{}]".format(file_path, container.name)
        )
...
def restore_file(container, file_path):
    ...
    if (
        execute.execute_command(
            container, "cp {} {}".format(backup_file_path, file_path)
        )
        != 0
    ):
        raise RuntimeError(
            "failed to restore [{}] [{}]".format(file_path, container.name)
        )


def configure_ssl_in_client(container, client_ssl_negotiation, irods_env=None):
    env = irods_env or json_utils.get_json_from_file(
        container, context.service_account_irods_env()
    )
...
def configure_ssl_in_server(container, server_ssl_negotiation):
    acPreConnect = 'acPreConnect(*OUT) {{ *OUT=\\"{}\\"; }}'.format(
...
def show_configurations(container, stream_output=False):
    ...
    show_irods_env = "bash -c \"cat {} | jq '.'\"".format(
        context.service_account_irods_env()
    )

    if (
        execute.execute_command(container, show_core_re, stream_output=stream_output)
        != 0
    ):
    ...
    if (
        execute.execute_command(
            container, show_server_config, stream_output=stream_output
        )
        != 0
    ):
    ...
    if (
        execute.execute_command(container, show_irods_env, stream_output=stream_output)
        != 0
    ):
        raise RuntimeError(
            "failed to cat irods_environment [{}]".format(container.name)
        )


def configure_negotiation_key(container, negotiation_key, server_config=None):
    config = server_config or json_utils.get_json_from_file(
