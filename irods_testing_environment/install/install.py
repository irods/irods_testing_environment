...
class installer(object):
    ...
    def get_list_of_package_paths(self, package_directory, package_name_list=None):
        ...
        if not package_directory:
            raise RuntimeError(
                "Attempting to install custom packages from unspecified location"
            )
        ...
        for p in package_name_list:
            glob_str = os.path.join(
                package_path, p + "*.{}".format(self.filename_extension())
            )
    ...
    def install_packages_on_container_from_tarfile(
        self, ctx, container_name, package_paths, tarfile_path
    ):
        ...
        package_list = " ".join(
            [
                ...
                if not context.is_database_plugin(p)
                or context.is_irods_catalog_provider_container(container)
            ]
        )
        ...
        logging.warning(
            "executing cmd [{0}] on container [{1}]".format(cmd, container.name)
        )

        ec = execute.execute_command(container, self.update_command())
        if ec != 0:
            logging.error(
                "failed to update local repositories [{}]".format(container.name)
            )
            return ec
        ...
        if ec != 0:
            logging.error(
                "failed to install packages on container [ec=[{0}], container=[{1}]".format(
                    ec, container.name
                )
            )
            return ec
    ...
    def install_packages(
        self, ctx, package_directory, containers, package_name_list=None
    ):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for f in concurrent.futures.as_completed(futures_to_containers):
                ...
                try:
                    ...
                    if ec != 0:
                        logging.error(
                            "error while installing packages on container [{}]".format(
                                container.name
                            )
                        )
                        rc = ec
                        ...
                        logging.info(
                            "packages installed successfully [{}]".format(
                                container.name
                            )
                        )
    ...
    def install_official_irods_packages(self, ctx, version, containers):
        ...
        if version:
            packages = [
                "{}{}{}".format(p, self.version_joinery(), version)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for f in concurrent.futures.as_completed(futures_to_containers):
                ...
                try:
                    ...
                    logging.info(
                        "packages installed successfully [{}]".format(container.name)
                    )
    ...
    def install_irods_packages(
        ...
        if package_directory:
            logging.warning(
                "installing iRODS packages from directory [{}]".format(
                    package_directory
                )
            )
            ...
            logging.warning(
                "installing official iRODS packages [{}]".format(package_version)
            )

            ec = self.install_official_irods_packages(
                ctx, package_version, ctx.irods_containers()
            )
            if ec != 0:
                raise RuntimeError("failed to install iRODS packages")
...
def install_pip_package_from_repo(
    container, repo_name, url_base="https://github.com/irods", branch=None
):
    ...
    repo_path = services.clone_repository_to_container(
        container, repo_name, url_base=url_base, branch=branch
    )
    ec = execute.execute_command(
        container,
        " ".join([container_info.python(container), "-m", "pip", "install", repo_path]),
    )
    if ec != 0:
        raise RuntimeError(
            "Failed to install pip package [{}] [{}]".format(repo_path, container.name)
        )
