...
class postgres_database_setup_strategy(database_setup_strategy):
    ...
    def create_user(self, username, password, force_recreate=False):
        ...
        return (
            ...
            else self.execute_psql_command(
    ...
    def grant_privileges(self, database, username):
        ...
        return self.execute_psql_command(
            'alter database \\"{0}\\" owner to {1};'.format(database, username)
        )
...
class mysql_database_setup_strategy(database_setup_strategy):
    ...
    def execute_mysql_command(self, mysql_cmd, user="root", password="testpassword"):
        ...
        """
        return execute.execute_command(
            self.container,
            '{0} --host 127.0.0.1 --port {1} --user {2} --password={3} --execute "{4}"'.format(
                self.db_exec, self.port, user, password, mysql_cmd
            ),
        )

    def connect_to_database(
        self, name="information_schema", as_user="root", with_password="testpassword"
    ):
        """Connect to database named `name` as user `as_user`.
        ...
        while self.execute_mysql_command("SHOW DATABASES;") != 0:
            logging.debug(
                "database is not accepting connections yet. retrying in 5 seconds ..."
            )
            time.sleep(5)
        ...
        return self.execute_mysql_command(
            "\\r '{}'".format(name), user=as_user, password=with_password
        )
    ...
    def create_user(self, username, password, force_recreate=False):
        ...
        return (
            ...
            if self.user_exists(username, password)
                ...
                "CREATE USER {} IDENTIFIED BY '{}';".format(user, password)
            )
        )
...
class mariadb_database_setup_strategy(mysql_database_setup_strategy):
    ...
    def __init__(self, container=None, root_password=None, port=None, db_exec=None):
        ...
        super(mariadb_database_setup_strategy, self).__init__(
            container, root_password, port, db_exec
        )

    def execute_mariadb_command(
        self, mariadb_cmd, user="root", password="testpassword"
    ):
...
def make_strategy(
    database_image, container=None, database_port=None, root_password=None
):
...
def setup_catalog(
    ...
    db_container = ctx.docker_client.containers.get(
        context.irods_catalog_database_container(
            ctx.compose_project.name, service_instance
        )
    )
    ...
    if ec != 0:
        raise RuntimeError(
            "failed to grant privileges to user [{0}] on database [{1}]".format(
...
def wait_for_database_service(
    ctx, database_service_instance=1, seconds_between_retries=1, retry_count=60
):
    ...
    db_container = ctx.docker_client.containers.get(
        context.irods_catalog_database_container(
            ctx.compose_project.name, database_service_instance
        )
    )
    ...
    while retries < retry_count:
        ...
        cmd = " ".join(
            [container_info.python(irods_container), "-c", f'"{socket_cmd}"']
        )
    ...
    raise RuntimeError(
        f"maximum retries reached attempting to connect to database [{db_container.name}]"
    )
