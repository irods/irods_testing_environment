...
class test_manager:
    ...
    def return_code(self):
        ...
        return (
            0
            if [tr.rc for tr in self.test_runners].count(0) == len(self.test_runners)
            else 1
        )
    ...
    def result_string(self):
        ...
        for tr in self.test_runners:
            ...
            tests_were_skipped = (
                ...
                else len(tr.skipped_tests()) > 0
            )

        if self.return_code() != 0:
            r = r + "List of failed tests:\n\t{}\n".format(
                " ".join([t or "all tests" for t, _ in self.failed_tests()])
            )
        ...
        if self.duration > 0:
            ...
            r = (
                r
                + "time elapsed: [{:>9.4f}]seconds ([{:>4d}]hours [{:>7.4f}]minutes)\n".format(
    ...
    def run(self, fail_fast=True, options=None, **kwargs):
        ...
        **kwargs -- keyword arguments to be passed to the `test_runner`'s specific `run` method
        ...
        if not isinstance(options, list) or len(options) != len(self.test_runners):
            raise ValueError(
                "options must be a list having a size equal to the number of concurrent executors"
            )
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures_to_test_runners = {
                executor.submit(
                    tr.run, test_queue, fail_fast, options=options[i], **kwargs
                ): tr
                for i, tr in enumerate(self.test_runners)
            }
