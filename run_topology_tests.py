...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run iRODS tests in a consistent environment."
    )
    ...
    ctx = context.context(
        docker.from_env(use_ssh_client=True),
        compose.cli.command.get_project(
            project_dir=project_directory, project_name=args.project_name
        ),
    )
    ...
    try:
        ...
        logging.debug(
            "got containers to run on [{}]".format(
        ...
        options_base.append(
            "--topology={}".format("resource" if run_on_consumer else "icat")
        )
        ...
        for i in range(args.executor_count):
            ...
            for c in ctx.compose_project.containers():
                ...
                consumer_service_instances_for_this_zone = range(
                    i * consumer_count + 1, (i + 1) * consumer_count + 1
                )
                logging.debug(
                    "consumer_service_instances_for_this_zone:[{}]".format(
                        [inst for inst in consumer_service_instances_for_this_zone]
                    )
                )
                container_is_consumer_for_this_zone = (
                    context.is_irods_catalog_consumer_container(c)
                    and container_service_instance
                    in consumer_service_instances_for_this_zone
                )
            ...
            hostnames_option.extend(
                [consumer_hostname_map[k] for k in sorted(consumer_hostname_map)]
            )
        ...
        rc = test_utils.run_specific_tests(
            containers, args.tests, options_list, args.fail_fast
        )
        ...
        if args.save_logs:
            try:
                ...
                logs.collect_logs(
                    ctx.docker_client, ctx.irods_containers(), output_directory
                )
